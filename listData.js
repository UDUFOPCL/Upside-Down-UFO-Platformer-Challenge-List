/*
All info in the list for each level

Level data includes:
    name : Name of the level in-game
    image : File path of the level image
    ID : ID for the level in-game
    creator : Name of the player who created the level
    verifier : Name of the player who verified the level
    recorder : Name of the player who currently holds the fastest completion for the level
    time : The time for the fastest completion of the level

Level Template:

    # : {
        name : '',
        image : 'Images/.',
        ID : ,
        creator : '',
        verifier : '',
        recorder : '',
        time : 'X:XX.XXX'
    },

*/
const data = {
    1 : {
        name : 'FALLEN ANGEL',
        image : 'Images/.fallenAngel.webp',
        ID : 131366052,
        creator : 'Astatine16',
        verifier : 'Astatine16',
        recorder : 'Astatine16',
        time : '18.599'
    },
    2 : {
        name : 'BrambleIfItWasGood',
        image : 'Images/.barmbleGood.webp',
        ID : 114998675,
        creator : 'ashfalcons44',
        verifier : 'ashfalcons44',
        recorder : 'ashfalcons44',
        time : '13.725'
    },
    3 : {
        name : 'noobedufopc ultra',
        image : 'Images/.noobedufopcUltra.webp',
        ID : 131546166,
        creator : 'ItsNoobe',
        verifier : 'ItsNoobe',
        recorder : 'ItsNoobe',
        time : '39.645'
    },
    4 : {
        name : 'Lament',
        image : 'Images/.lament.webp',
        ID : 114988101,
        creator : 'tenkuyuhh',
        verifier : 'tenkuyuhh',
        recorder : 'tenkuyuhh',
        time : '22.520'
    },
    5 : {
        name : 'BlUDUFObath',
        image : 'Images/.bath.webp',
        ID : 114965788,
        creator : 'rush',
        verifier : 'rush',
        recorder : 'rush',
        time : '2:45.383'
    },
    6 : {
        name : 'run from emoji9',
        image : 'Images/.runFromEmoji9.webp',
        ID : 119771687,
        creator : 'ItsNoobe',
        verifier : 'ItsNoobe',
        recorder : 'ItsNoobe',
        time : '19.066'
    },
    7 : {
        name : 'launchpad ufobyrinth',
        image : 'Images/.launchpad.webp',
        ID : 127331837,
        creator : 'ItsNoobe',
        verifier : 'ItsNoobe',
        recorder : 'ItsNoobe',
        time : '2.712'
    },
    8 : {
        name : 'Fever',
        image : 'Images/.fever.webp',
        ID : 114963741,
        creator : 'tenkuyuhh',
        verifier : 'tenkuyuhh',
        recorder : 'tenkuyuhh',
        time : '17.400'
    },
    9 : {
        name : 'Suffer',
        image : 'Images/.suffer.webp',
        ID : 111863467,
        creator : 'tenkuyuhh',
        verifier : 'tenkuyuhh',
        recorder : 'tenkuyuhh',
        time : '21.912'
    },
    10 : {
        name : 'agony',
        image : 'Images/.agony.webp',
        ID : 114960048,
        creator : 'tenkuyuhh',
        verifier : 'tenkuyuhh',
        recorder : 'tenkuyuhh',
        time : '19.166'
    },
    11 : {
        name : 'mangoa',
        image : 'Images/.mangoa.webp',
        ID : 114952534,
        creator : 'tenkuyuhh',
        verifier : 'tenkuyuhh',
        recorder : 'MyCat223',
        time : '15.812'
    },
    12 : {
        name : 'VSC but ufo',
        image : 'Images/.vscUfo.webp',
        ID : 114984854,
        creator : 'CoinvesselGD',
        verifier : 'tenkuyuhh',
        recorder : 'tenkuyuhh',
        time : '35.791'
    },
    13 : {
        name : 'Mycat sigma boy chal',
        image : 'Images/.sigmaBoy.webp',
        ID : 114984412,
        creator : 'tenkuyuhh',
        verifier : 'tenkuyuhh',
        recorder : 'tenkuyuhh',
        time : '17.754'
    },
    14 : {
        name : 'spam challenge 1',
        image : 'Images/.spam1.webp',
        ID : 114952847,
        creator : 'tenkuyuhh',
        verifier : 'tenkuyuhh',
        recorder : 'tenkuyuhh',
        time : '20.091'
    },
    15 : {
        name : 'RENEWAL',
        image : 'Images/.renewal.webp',
        ID : 118612227,
        creator : 'ItsNoobe',
        verifier : 'ItsNoobe',
        recorder : 'ItsNoobe',
        time : '29.641'
    },
    16 : {
        name : 'UFO Funk',
        image : 'Images/.ufoFunk.webp',
        ID : 124325607,
        creator : 'CoinvesselGD',
        verifier : 'Astatine16',
        recorder : 'Astatine16',
        time : '14.000'
    },
    17 : {
        name : 'average udufopcl level',
        image : 'Images/.average.webp',
        ID : 114935113,
        creator : 'MyCat223',
        verifier : 'MyCat223',
        recorder : 'MyCat223',
        time : '37.762'
    },
    18 : {
        name : 'zimbabwe',
        image : 'Images/.zimbabwe.webp',
        ID : 115127631,
        creator : 'TheNewCodex',
        verifier : 'TheNewCodex',
        recorder : 'TheNewCodex',
        time : '9.166'
    },
    19 : {
        name : 'TOOMDUDUFOIGDPCLBM',
        image : 'Images/.toomdud.webp',
        ID : 114955327,
        creator : 'Codex',
        verifier : 'Codex',
        recorder : 'Codex',
        time : '9.083'
    },
    20 : {
        name : 'le funky ufo level',
        image : 'Images/.leFunky.webp',
        ID : 114952921,
        creator : 'ItsNoobe',
        verifier : 'ItsNoobe',
        recorder : 'rush',
        time : '29.770'
    },
};